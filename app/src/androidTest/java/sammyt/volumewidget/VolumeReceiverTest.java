...
public class VolumeReceiverTest {
    ...
    public void testMediaVolumeDecrease() {
        ...
        if(beforeValue > 0) {
            Log.i(LOG_TAG, "Decreased from " + beforeValue + " to " + afterValue);
            assertTrue(beforeValue > afterValue);
        }else{
            Log.i(LOG_TAG, "Decreased from 0 volume. Value remains the same.");
    ...
    public void testMediaVolumeIncrease() {
        ...
        if(beforeValue < audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC)) {
            Log.i(LOG_TAG, "Increased from " + beforeValue + " to " + afterValue);
            assertTrue(beforeValue < afterValue);
        }else{
            Log.i(LOG_TAG, "Increased from max volume. Value remains the same.");
            assertEquals(beforeValue, afterValue);
        }
    }
}
